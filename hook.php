...
if ($POST['message']['chat']['type'] == 'group') {
    if (preg_match('/asd/', $POST['message']['text'])) {
        ...
        $result = $conn->query($sql);
            ...
            $aa = $conn->query($sql);
                $sql = "SELECT SUM(num) AS fr
            FROM asd";
          $row = mysqli_fetch_object($aa);
         $asdco = $row->fr;
        switch($asdco->num_rows) {
...
if ($POST['message']['chat']['type'] == 'private' && $POST['message']['text'] != '/grafico') {
...
if ($POST['message']['chat']['type'] == 'group' && $POST['message']['text'] == '/grafico') {
